...
public:
    ...
    node_schedule(cluster_node const & node_) :
        node{node_} {}

    // returns EFT and iterator before which the task could be scheduled
    eft_position compute_earliest_finish_time(
        ...
        if (curr_it == intervals.end()) {
            ...
            return eft_position{earliest_start_time + computation_time, intervals.end()};
        }
        ...
        if (curr_it == intervals.begin() && curr_it->start >= ready_time + computation_time) {
            ...
            return eft_position{computation_time, curr_it};
        }
        ...
        while (true) {
            ...
            if (
                ...
                return eft_position{curr_it->end + computation_time, next_it};
            }
    ...
    cluster_node const & get_node() const {
        return node;
    }

    double get_computation_time(task const & t) const {
